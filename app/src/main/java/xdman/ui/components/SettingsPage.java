...
public class SettingsPage extends JPanel implements ActionListener, ListSelectionListener {
    ...
    private JPanel createOverviewPanel() {
        ...
        h = getScaledInt(25);
        cmbZoom = new JComboBox<>(XDMApp.ZOOM_LEVEL_STRINGS);
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (e.getSource() instanceof JCheckBox) {
            ...
            if ("FF_INSTALL".equals(name)) {
                OS.installNativeMessagingHost(Browser.FIREFOX);
                if (!OS.launchBrowser(Browser.FIREFOX, ffAMOURL)) {
            ...
            if ("EDGE_INSTALL".equals(name)) {
                OS.browseURL("https://sourceforge.net/p/xdman/blog/2018/01/xdm-integration-with-microsoft-edge/");
            }
            if ("CR_INSTALL".equals(name)) {
                OS.installNativeMessagingHost(Browser.CHROME);
                if (!OS.launchBrowser(Browser.CHROME, chromeWebStoreURL)) {
            ...
            if ("CM_INSTALL".equals(name)) {
                OS.installNativeMessagingHost(Browser.CHROMIUM);
    ...
    private void loadPasswordSettings() {
        ...
        for (Entry<String, PasswordAuthentication> ent : credentials) {
    ...
    private JPanel createPasswordPanel() {
        ...
        passList.addListSelectionListener(e -> {
            int index = passList.getSelectedIndex();
    ...
    private JPanel createAdvPanel() {
        ...
        y += h;
        y += getScaledInt(50);
        chkShowTray.setVisible(XDMUtils.detectOS() == XDMUtils.LINUX);
    ...
    private void loadAdvSettings() {
        ...
        chkAutoStart.setSelected(OperationSystem.OS.isAlreadyAutoStart());
    ...
    private void saveAdvSettings() {
        ...
        if (chkAutoStart.isSelected()) {
            OperationSystem.OS.addToStartup();
        } else {
            OperationSystem.OS.removeFromStartup();
        }
