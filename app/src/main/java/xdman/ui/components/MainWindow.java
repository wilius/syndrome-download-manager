    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (e.getSource() instanceof JComponent) {
            ...
            if (name.startsWith("STOP")) {
                ...
                if (ids.length > 0) {
                    String id = ids[0];
                    OS.copyURL(XDMApp.getInstance().getURL(id));
                }
                ...
                config.setCategoryFilter(FileCategory.DOCUMENT);
                updateSidePanel((JLabel) e.getSource());
            } else if ("CAT_COMPRESSED".equals(name)) {
                config.setCategoryFilter(FileCategory.COMPRESSED);
                updateSidePanel((JLabel) e.getSource());
            } else if ("CAT_MUSIC".equals(name)) {
                config.setCategoryFilter(FileCategory.MUSIC);
                updateSidePanel((JLabel) e.getSource());
            } else if ("CAT_PROGRAMS".equals(name)) {
                config.setCategoryFilter(FileCategory.APP);
                updateSidePanel((JLabel) e.getSource());
            } else if ("CAT_VIDEOS".equals(name)) {
                config.setCategoryFilter(FileCategory.VIDEO);
                updateSidePanel((JLabel) e.getSource());
            } else if ("CAT_ALL".equals(name)) {
                config.setCategoryFilter(null);
                updateSidePanel((JLabel) e.getSource());
                ...
                convert();
            } else if ("MENU_RESTART_DWN".equals(name)) {
                ...
                OS.browseURL(XDMApp.APP_WIKI_URL);
            } else if ("MENU_HOME_PAGE".equals(name)) {
                OS.browseURL(XDMApp.APP_HOME_URL);
            } else if ("MENU_UPDATE".equals(name)) {
                OS.browseURL(XDMApp.APP_UPDATE_CHK_URL + XDMApp.APP_VERSION);
...
//		getTitlePanel().add(panel);
    ...
    private void createPopupMenu() {
        ...
        addMenuItem("MENU_RESTART_DWN", popupCtx);
    ...
    public void openFile(String id) {
        ...
        if (ent != null) {
            if (ent.getState() == XDMConstants.FINISHED) {
                try {
                    OperationSystem.OS.openFile(new File(XDMApp.getInstance().getFolder(ent), ent.getFile()));
    ...
    public void openFolder(String id) {
        ...
        if (ent != null) {
            if (ent.getState() == XDMConstants.FINISHED) {
                try {
                    OperationSystem.OS.openFolder(XDMApp.getInstance().getFolder(ent), ent.getFile());
                } catch (FileNotFoundException e) {
                    Logger.log(e);
                    MessageBox.show(this, StringResource.get("ERR_MSG_FILE_NOT_FOUND"),
                            StringResource.get("ERR_MSG_FILE_NOT_FOUND_MSG"), MessageBox.OK, MessageBox.OK);
    ...
    private void openTranslationPage() {
        OS.browseURL("https://github.com/subhra74/xdm/wiki/Submitting-translations-for-XDM");
    }

    private void openSupportPage() {
        OS.browseURL("https://github.com/subhra74/xdm/wiki");
    }

    private void openBugReportPage() {
        OS.browseURL("https://github.com/subhra74/xdm/issues");
    }
